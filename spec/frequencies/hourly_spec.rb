  ...
  describe '#next_occurrences' do
    ...
    context 'with an interval of one' do
      ...
      it 'returns sequential hours' do
        ...
        expect(frequency.next_occurrences).to eql [Time.zone.local(1997, 1, 1, 1, 0, 0)]
        expect(frequency.next_occurrences).to eql [Time.zone.local(1997, 1, 1, 2, 0, 0)]
      end
    end
    ...
    context 'with an interval of two' do
      ...
      it 'returns every 2 hours' do
        expect(frequency.next_occurrences).to eql [Time.zone.local(1997, 1, 1, 0, 0, 0)]
        expect(frequency.next_occurrences).to eql [Time.zone.local(1997, 1, 1, 2, 0, 0)]
        expect(frequency.next_occurrences).to eql [Time.zone.local(1997, 1, 1, 4, 0, 0)]
      end
    end
  end
